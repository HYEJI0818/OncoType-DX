  ...
  */
  ...
  const loadTumorFromSegFile = async () => {
    try {
      ...
      if (!sessionId) {
        console.log('📝 Flask 서버 비활성화 - OncotypeMPRViewer seg 파일 로드 건너뜀');
        return;
      }

      console.log('📝 Flask 서버 비활성화 - OncotypeMPRViewer seg 파일 로드 기능 사용 안함');
      
      // Flask 서버가 비활성화되어 있으므로 seg 파일 로드하지 않음
      
    } catch (error) {
      console.log('📝 Flask 서버 비활성화 - OncotypeMPRViewer Tumor 로드 기능 사용 안함');
    }
  };
